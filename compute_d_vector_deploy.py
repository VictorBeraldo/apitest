...
model_file='model_raw.pkl' # This is the model to use for computing the d-vectors (it should be pre-trained using the speaker-id DNN)
cfg_file='cfg/SincNet_Librispeech _win.cfg' # Config file of the speaker-id experiment used to generate the model
te_lst='data_split/libri_tr_deploy.scp' # List of the wav files to process
out_dict_file='d_vect_libri.npy' # output dictionary containing the a sentence id as key as the d-vector as value
data_folder='converted'
...
# torch.load(model_file) # for cuda
checkpoint_load = torch.load(model_file,map_location='cpu') 
...
# Escolher porta e host    
if __name__ == "__main__":
    app.run(host="0.0.0.0", port = 5000, debug=True)    
    
#curl --request POST --url http://0.0.0.0:5000/predict_topic --header 'content-type: application/json' --data '{"rawtext_list":"1001-134707-0004-4.wav"}'
